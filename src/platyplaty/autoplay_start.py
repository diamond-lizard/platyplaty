...
async def load_first_preset(
    ctx: "AppContext", playlist: "Playlist", first_index: int
) -> bool:
    ...
    success, error = await try_load_preset(ctx, preset_path)
    if success or error is None:
        return success
    playlist.broken_indices.add(first_index)
    ctx.error_log.append(error)
    return await try_advance_after_error(ctx, playlist)


async def try_advance_after_error(ctx: "AppContext", playlist: "Playlist") -> bool:
...
async def start_from_first_preset(ctx: "AppContext", playlist: "Playlist") -> bool:
    ...
    return await load_first_preset(ctx, playlist, first_playable)
